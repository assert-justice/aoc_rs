...
impl Parser{
    ...
    fn parse(txt: &str) -> Vec<Token>{
        ...
        while !parser.at_eof() {
            if parser.match_str("do()"){
                parser.tokens.push(Token::Start);
                continue;
            }
            if parser.match_str("don't()"){
                parser.tokens.push(Token::Stop);
                continue;
            }
...
pub fn solve(txt: &str){
    ...
    for t in tokens {
        match t {
            ...
            Token::Start => {enabled = true;}
            Token::Stop => {enabled = false;}
        }
    }
    println!("{res}");
}
